...
var require_tiny_emitter = __commonJS({
  "node_modules/tiny-emitter/index.js"(exports, module) {
    ...
    E.prototype = {
      on: function(name118, callback, ctx) {
        var e2 = this.e || (this.e = {});
        (e2[name118] || (e2[name118] = [])).push({
          fn: callback,
          ctx
        });
        return this;
      },
      once: function(name118, callback, ctx) {
        var self2 = this;
        function listener() {
          self2.off(name118, listener);
          callback.apply(ctx, arguments);
        }
        ;
        listener._ = callback;
        return this.on(name118, listener, ctx);
      },
      emit: function(name118) {
        var data = [].slice.call(arguments, 1);
        var evtArr = ((this.e || (this.e = {}))[name118] || []).slice();
        var i = 0;
      ...
      off: function(name118, callback) {
        ...
        liveEvents.length ? e2[name118] = liveEvents : delete e2[name118];
        return this;
      }
    };
    module.exports = E;
...
function validateOption(options, name118, values) {
  if (options[name118] !== void 0 && !values.includes(options[name118])) {
    console.warn('Warning: Unknown value "' + options[name118] + '" for configuration option "' + name118 + '". Available options: ' + values.map((value) => JSON.stringify(value)).join(", ") + ".");
  }
}
...
function factory(name118, dependencies119, create3, meta) {
  function assertAndCreate(scope) {
    var deps = pickShallow(scope, dependencies119.map(stripOptionalNotation));
    assertDependencies(name118, dependencies119, scope);
    return create3(deps);
  }
  assertAndCreate.isFactory = true;
  assertAndCreate.fn = name118;
  assertAndCreate.dependencies = dependencies119.slice().sort();
  if (meta) {
...
function assertDependencies(name118, dependencies119, scope) {
  var allDefined = dependencies119.filter((dependency) => !isOptionalDependency(dependency)).every((dependency) => scope[dependency] !== void 0);
  if (!allDefined) {
    var missingDependencies = dependencies119.filter((dependency) => scope[dependency] === void 0);
    throw new Error('Cannot create function "'.concat(name118, '", ') + "some dependencies are missing: ".concat(missingDependencies.map((d) => '"'.concat(d, '"')).join(", "), "."));
  }
}
...
function create() {
  ...
  function findType(typeName) {
    ...
    const name118 = typeName.toLowerCase();
    let otherName;
    for (otherName of typeList) {
      if (otherName.toLowerCase() === name118) {
  ...
  function findTypeNames(value) {
    const matches = typeList.filter((name118) => {
      const type = typeMap.get(name118);
  ...
  function findSignature(fn, signature, options) {
    ...
    if (exact) {
      remainingSignatures = [];
      let name118;
      for (name118 in fn.signatures) {
        remainingSignatures.push(fn._typedFunctionData.signatureMap.get(name118));
      }
    } else {
  ...
  function mergeExpectedParams(signatures, index) {
    ...
    signatures.forEach((signature) => {
      ...
      let name118;
      for (name118 of paramSet) {
        typeSet.add(name118);
      }
    });
    return typeSet.has("any") ? ["any"] : Array.from(typeSet);
  }
  function createError(name118, args, signatures) {
    let err, expected;
    const _name = name118 || "unnamed";
  ...
  function conflicting(params1, params2) {
    ...
    for (let i = 0; i < ii; i++) {
      ...
      let overlap = false;
      let name118;
      for (name118 of typeSet2) {
        if (typeSet1.has(name118)) {
          overlap = true;
          break;
        }
      }
  ...
  function createTypedFunction(name118, rawSignaturesMap) {
    typed.createCount++;
    ...
    const generic = function generic2() {
      ...
      return typed.onMismatch(name118, arguments, signatures);
    };
  ...
  function _onMismatch(name118, args, signatures) {
    throw createError(name118, args, signatures);
  }
  ...
  function getObjectName(obj) {
    ...
    for (const key in obj) {
      if (Object.prototype.hasOwnProperty.call(obj, key) && (isTypedFunction(obj[key]) || typeof obj[key].signature === "string")) {
        name118 = checkName(name118, obj[key].name);
      }
    }
    return name118;
  }
  ...
  typed = function(maybeName) {
    ...
    for (let i = start; i < arguments.length; ++i) {
      ...
      if (!named) {
        name118 = checkName(name118, thisName);
      }
      mergeSignatures(allSignatures, theseSignatures);
    }
    return createTypedFunction(name118 || "", allSignatures);
  };
  typed.create = create;
...
var createTyped = factory("typed", dependencies, function createTyped2(_ref) {
  ...
  typed.onMismatch = (name118, args, signatures) => {
    var usualError = typed.createError(name118, args, signatures);
    ...
    signatures.some((sig) => !sig.params.includes(","))) {
      var err = new TypeError("Function '".concat(name118, "' doesn't apply to matrices. To call it ") + "elementwise on a matrix 'M', try 'map(M, ".concat(name118, ")'."));
...
var createChainClass = factory(name4, dependencies5, (_ref) => {
  ...
  function createProxy(name118, fn) {
    if (typeof fn === "function") {
      Chain.prototype[name118] = chainify(fn);
    }
  }
  function createLazyProxy(name118, resolver) {
    lazy(Chain.prototype, name118, function outerResolver() {
      var fn = resolver();
      if (typeof fn === "function") {
        return chainify(fn);
      }
  ...
  if (on) {
    on("import", function(name118, resolver, path) {
      if (!path) {
        createLazyProxy(name118, resolver);
      }
    });
  }
  return Chain;
...
var createUtil = factory(name8, dependencies9, (_ref) => {
  ...
  function hasProperty(nodeOrName, property) {
    ...
    var name118 = defaultName;
    if (typeof nodeOrName === "string") {
      name118 = nodeOrName;
    } else if (isOperatorNode(nodeOrName)) {
      name118 = nodeOrName.fn.toString();
    } else if (isFunctionNode(nodeOrName)) {
      name118 = nodeOrName.name;
    } else if (isParenthesisNode(nodeOrName)) {
      name118 = "paren";
    }
    if (hasOwnProperty(context, name118)) {
      var properties2 = context[name118];
      if (hasOwnProperty(properties2, property)) {
        return properties2[property];
      }
      if (hasOwnProperty(defaultContext, name118)) {
        return defaultContext[name118][property];
      }
    }
    ...
    if (hasOwnProperty(defaultContext, name118)) {
      var _properties2 = defaultContext[name118];
...
var dependencies11 = ["typed", "config", "mathWithTransform", "matrix", "?fraction", "?bignumber", "AccessorNode", "ArrayNode", "ConstantNode", "FunctionNode", "IndexNode", "ObjectNode", "OperatorNode", "SymbolNode"];
...
var createCeilNumber = factory(name17, ["typed", "config", "round"], (_ref) => {
  ...
  return typed(name17, {
    number: function number(x) {
      if (nearlyEqual(x, round2(x), config4.relTol, config4.absTol)) {
        return round2(x);
      } else {
        return Math.ceil(x);
      }
    ...
    "number, number": function number_number(x, n) {
      if (nearlyEqual(x, round2(x, n), config4.relTol, config4.absTol)) {
        return round2(x, n);
      } else {
...
var createCeil = factory(name17, dependencies18, (_ref2) => {
  ...
  return typed("ceil", {
    ...
    BigNumber: function BigNumber(x) {
      if (nearlyEqual2(x, round2(x), config4.relTol, config4.absTol)) {
        return round2(x);
      } else {
        return x.ceil();
      }
    ...
    "BigNumber, BigNumber": function BigNumber_BigNumber(x, n) {
      if (nearlyEqual2(x, round2(x, n), config4.relTol, config4.absTol)) {
        return round2(x, n);
      } else {
...
var dependencies22 = ["typed", "addScalar", "subtractScalar", "multiplyScalar", "divideScalar", "pow", "factorial", "combinations", "isNegative", "isInteger", "number", "?bignumber", "larger"];
...
function recreateFactory(name118, dependencies119, create3) {
  return factory(name118, dependencies119, create3, {
...
var createBigint = factory(name26, dependencies27, (_ref) => {
  var {
    typed
  } = _ref;
...
var createString = factory(name27, dependencies28, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name27, {
...
var createBoolean = factory(name28, dependencies29, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name28, {
...
var name29 = "parser";
var dependencies30 = ["typed", "Parser"];
var createParser = factory(name29, dependencies30, (_ref) => {
  var {
    typed,
    Parser
  } = _ref;
...
var name30 = "Node";
var dependencies31 = ["mathWithTransform"];
var createNode = factory(name30, dependencies31, (_ref) => {
  var {
    mathWithTransform
  } = _ref;
...
var name31 = "AccessorNode";
var dependencies32 = ["subset", "Node"];
var createAccessorNode = factory(name31, dependencies32, (_ref) => {
  var {
    subset,
    Node
  } = _ref;
  ...
  _defineProperty(AccessorNode, "name", name31);
  return AccessorNode;
}, {
  isClass: true,
  isNode: true
...
var name32 = "ArrayNode";
var dependencies33 = ["Node"];
var createArrayNode = factory(name32, dependencies33, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(ArrayNode, "name", name32);
  return ArrayNode;
}, {
  isClass: true,
  isNode: true
...
var createAssignmentNode = factory(name33, dependencies34, (_ref) => {
  var {
    subset,
    matrix,
    Node
    ...
    _compile(math, argNames) {
      ...
      if (!this.index) {
        ...
        return function evalAssignmentNode(scope, args, context) {
          ...
          scope.set(name118, assign(childObject, index, value));
          return value;
        };
      } else {
  ...
  _defineProperty(AssignmentNode, "name", name33);
  return AssignmentNode;
}, {
  isClass: true,
  isNode: true
...
var name34 = "BlockNode";
var dependencies35 = ["ResultSet", "Node"];
var createBlockNode = factory(name34, dependencies35, (_ref) => {
  var {
    ResultSet,
    Node
  } = _ref;
  ...
  _defineProperty(BlockNode, "name", name34);
  return BlockNode;
}, {
  isClass: true,
  isNode: true
...
var name35 = "ConditionalNode";
var dependencies36 = ["Node"];
var createConditionalNode = factory(name35, dependencies36, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(ConditionalNode, "name", name35);
  return ConditionalNode;
}, {
  isClass: true,
  isNode: true
...
function toSymbol(name118, isUnit2) {
  isUnit2 = typeof isUnit2 === "undefined" ? false : isUnit2;
  if (isUnit2) {
    if (hasOwnProperty(latexUnits, name118)) {
      return latexUnits[name118];
    }
    return "\\mathrm{" + escapeLatex(name118) + "}";
  }
  if (hasOwnProperty(latexSymbols, name118)) {
    return latexSymbols[name118];
  }
  return escapeLatex(name118);
}

// node_modules/mathjs/lib/esm/expression/node/ConstantNode.js
var name36 = "ConstantNode";
var dependencies37 = ["Node"];
var createConstantNode = factory(name36, dependencies37, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(ConstantNode, "name", name36);
  return ConstantNode;
}, {
  isClass: true,
  isNode: true
...
var name37 = "FunctionAssignmentNode";
var dependencies38 = ["typed", "Node"];
var createFunctionAssignmentNode = factory(name37, dependencies38, (_ref) => {
  var {
    typed,
    Node
  } = _ref;
  ...
  class FunctionAssignmentNode extends Node {
     ...
     */
    constructor(name118, params, expr) {
      super();
      if (typeof name118 !== "string") {
      ...
      if (keywords.has(name118)) {
        throw new Error('Illegal function name, "' + name118 + '" is a reserved keyword');
      }
    ...
    _compile(math, argNames) {
      ...
      var name118 = this.name;
      var params = this.params;
      var signature = join(this.types, ",");
      var syntax = name118 + "(" + join(this.params, ", ") + ")";
      return function evalFunctionAssignmentNode(scope, args, context) {
        ...
        var fn = typed(name118, signatures);
        fn.syntax = syntax;
        scope.set(name118, fn);
        return fn;
      };
    }
    /**
  ...
  _defineProperty(FunctionAssignmentNode, "name", name37);
  return FunctionAssignmentNode;
}, {
  isClass: true,
  isNode: true
...
var name38 = "IndexNode";
var dependencies39 = ["Node", "size"];
var createIndexNode = factory(name38, dependencies39, (_ref) => {
  var {
    Node,
    size
  } = _ref;
  ...
  _defineProperty(IndexNode, "name", name38);
  return IndexNode;
}, {
  isClass: true,
  isNode: true
...
var name39 = "ObjectNode";
var dependencies40 = ["Node"];
var createObjectNode = factory(name39, dependencies40, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(ObjectNode, "name", name39);
  return ObjectNode;
}, {
  isClass: true,
  isNode: true
...
var name40 = "OperatorNode";
var dependencies41 = ["Node"];
var createOperatorNode = factory(name40, dependencies41, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(OperatorNode, "name", name40);
  return OperatorNode;
}, {
  isClass: true,
  isNode: true
...
var name41 = "ParenthesisNode";
var dependencies42 = ["Node"];
var createParenthesisNode = factory(name41, dependencies42, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(ParenthesisNode, "name", name41);
  return ParenthesisNode;
}, {
  isClass: true,
  isNode: true
...
var name42 = "RangeNode";
var dependencies43 = ["Node"];
var createRangeNode = factory(name42, dependencies43, (_ref) => {
  var {
    Node
  } = _ref;
  ...
  _defineProperty(RangeNode, "name", name42);
  return RangeNode;
}, {
  isClass: true,
  isNode: true
...
var name43 = "RelationalNode";
var dependencies44 = ["Node"];
var createRelationalNode = factory(name43, dependencies44, (_ref) => {
  var {
    Node
  } = _ref;
  var operatorMap = {
  ...
  _defineProperty(RelationalNode, "name", name43);
  return RelationalNode;
}, {
  isClass: true,
  isNode: true
...
var name44 = "SymbolNode";
var dependencies45 = ["math", "?Unit", "Node"];
var createSymbolNode = factory(name44, dependencies45, (_ref) => {
  var {
    math,
    Unit,
    Node
  } = _ref;
  function isValuelessUnit(name118) {
    return Unit ? Unit.isValuelessUnit(name118) : false;
  }
  class SymbolNode extends Node {
    ...
    _compile(math2, argNames) {
      var name118 = this.name;
      if (argNames[name118] === true) {
        return function(scope, args, context) {
          return getSafeProperty(args, name118);
        };
      } else if (name118 in math2) {
        return function(scope, args, context) {
          return scope.has(name118) ? scope.get(name118) : getSafeProperty(math2, name118);
        };
      } else {
        var isUnit2 = isValuelessUnit(name118);
        return function(scope, args, context) {
          return scope.has(name118) ? scope.get(name118) : isUnit2 ? new Unit(null, name118) : SymbolNode.onUndefinedSymbol(name118);
        };
      }
    }
    /**
     ...
     * @param {string} name
     */
    static onUndefinedSymbol(name118) {
      throw new Error("Undefined symbol " + name118);
    }
    /**
     * Create a clone of this node, a shallow copy
     * @return {SymbolNode}
     ...
     */
    _toHTML(options) {
      var name118 = escape(this.name);
      if (name118 === "true" || name118 === "false") {
        return '<span class="math-symbol math-boolean">' + name118 + "</span>";
      } else if (name118 === "i") {
        return '<span class="math-symbol math-imaginary-symbol">' + name118 + "</span>";
      } else if (name118 === "Infinity") {
        return '<span class="math-symbol math-infinity-symbol">' + name118 + "</span>";
      } else if (name118 === "NaN") {
        return '<span class="math-symbol math-nan-symbol">' + name118 + "</span>";
      } else if (name118 === "null") {
        return '<span class="math-symbol math-null-symbol">' + name118 + "</span>";
      } else if (name118 === "undefined") {
        return '<span class="math-symbol math-undefined-symbol">' + name118 + "</span>";
      }
      return '<span class="math-symbol">' + name118 + "</span>";
    }
    /**
     * Get a JSON representation of the node
     * @returns {Object}
...
var name45 = "FunctionNode";
var dependencies46 = ["math", "Node", "SymbolNode"];
var createFunctionNode = factory(name45, dependencies46, (_ref) => {
  var _FunctionNode;
  var {
    math,
    Node,
  ...
  class FunctionNode extends Node {
    ...
    toString(options) {
      var customString;
      var name118 = this.fn.toString(options);
      if (options && typeof options.handler === "object" && hasOwnProperty(options.handler, name118)) {
        customString = options.handler[name118](this, options);
      }
      if (typeof customString !== "undefined") {
        return customString;
      }
  ...
  _defineProperty(FunctionNode, "name", name45);
  _defineProperty(FunctionNode, "onUndefinedFunction", function(name118) {
    throw new Error("Undefined function " + name118);
  });
...
var name46 = "parse";
var dependencies47 = ["typed", "numeric", "config", "AccessorNode", "ArrayNode", "AssignmentNode", "BlockNode", "ConditionalNode", "ConstantNode", "FunctionAssignmentNode", "FunctionNode", "IndexNode", "ObjectNode", "OperatorNode", "ParenthesisNode", "RangeNode", "RelationalNode", "SymbolNode"];
var createParse = factory(name46, dependencies47, (_ref) => {
  var {
    typed,
    numeric: numeric2,
    config: config4,
  ...
  function parseAssignment(state) {
    ...
    if (state.token === "=") {
      if (isSymbolNode(node)) {
        ...
        return new AssignmentNode(new SymbolNode(name118), value);
        ...
        if (valid) {
          getTokenSkipNewline(state);
          value = parseAssignment(state);
          return new FunctionAssignmentNode(name118, args, value);
        }
      }
  ...
  function parseShift(state) {
    ...
    while (hasOwnProperty(operators, state.token)) {
      name118 = state.token;
      fn = operators[name118];
      getTokenSkipNewline(state);
      params = [node, parseConversion(state)];
      node = new OperatorNode(name118, fn, params);
    }
    return node;
  }
  ...
  function parseMultiplyDivide(state) {
    ...
    while (true) {
      if (hasOwnProperty(operators, state.token)) {
        ...
        node = new OperatorNode(name118, fn, [node, last]);
      } else {
        break;
      }
    }
  ...
  function parseModulusPercentage(state) {
    ...
    while (hasOwnProperty(operators, state.token)) {
      ...
      fn = operators[name118];
      getTokenSkipNewline(state);
      if (name118 === "%" && state.tokenType === TOKENTYPE.DELIMITER && state.token !== "(") {
  ...
  function parseUnary(state) {
    ...
    if (hasOwnProperty(operators, state.token)) {
      ...
      return new OperatorNode(name118, fn, params);
    }
  ...
  function parseSymbol(state) {
    ...
    if (state.tokenType === TOKENTYPE.SYMBOL || state.tokenType === TOKENTYPE.DELIMITER && state.token in NAMED_DELIMITERS) {
      name118 = state.token;
      getToken(state);
      if (hasOwnProperty(CONSTANTS, name118)) {
        node = new ConstantNode(CONSTANTS[name118]);
      } else if (NUMERIC_CONSTANTS.includes(name118)) {
        node = new ConstantNode(numeric2(name118, "number"));
      } else {
        node = new SymbolNode(name118);
      }
      node = parseAccessors(state, node);
      return node;
    }
...
var createCompile = factory(name47, dependencies48, (_ref) => {
  var {
    typed,
    parse
  } = _ref;
...
var name48 = "evaluate";
var dependencies49 = ["typed", "parse"];
var createEvaluate = factory(name48, dependencies49, (_ref) => {
  var {
    typed,
    parse
  } = _ref;
...
var name49 = "Parser";
var dependencies50 = ["evaluate"];
var createParserClass = factory(name49, dependencies50, (_ref) => {
  var {
    evaluate
  } = _ref;
  function Parser() {
  ...
  Parser.prototype.get = function(name118) {
    if (this.scope.has(name118)) {
      return this.scope.get(name118);
    }
  };
  ...
  Parser.prototype.set = function(name118, value) {
    this.scope.set(name118, value);
    return value;
  };
  Parser.prototype.remove = function(name118) {
    this.scope.delete(name118);
  };
...
var name50 = "apply";
var dependencies51 = ["typed", "isInteger"];
var createApply = factory(name50, dependencies51, (_ref) => {
  var {
    typed,
    isInteger: isInteger2
  } = _ref;
...
var createFilter = factory(name51, dependencies52, (_ref) => {
  var {
    typed
  } = _ref;
  return typed("filter", {
...
var createForEach = factory(name52, dependencies53, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name52, {
...
var createMap2 = factory(name53, dependencies54, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name53, {
  ...
  function _mapMultiple(Arrays, multiCallback) {
    ...
    function _getTypedCallbackCase(callback2, values, idx, arrays) {
      if (typed.resolve(callback2, [...values, idx, ...arrays]) !== null) {
        return 2;
      }
      if (typed.resolve(callback2, [...values, idx]) !== null) {
        return 1;
      }
      if (typed.resolve(callback2, values) !== null) {
        return 0;
      }
      return 0;
    }
...
var name54 = "range";
var dependencies55 = ["typed", "config", "?matrix", "?bignumber", "smaller", "smallerEq", "larger", "largerEq", "add", "isPositive"];
var createRange = factory(name54, dependencies55, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
...
var name55 = "size";
var dependencies56 = ["typed", "config", "?matrix"];
var createSize = factory(name55, dependencies56, (_ref) => {
  var {
    typed,
    config: config4,
    matrix
...
var name56 = "partitionSelect";
var dependencies57 = ["typed", "isNumeric", "isNaN", "compare"];
var createPartitionSelect = factory(name56, dependencies57, (_ref) => {
  var {
    typed,
    isNumeric,
    isNaN: isNaN2,
...
var name57 = "combinationsWithRep";
var dependencies58 = ["typed"];
var createCombinationsWithRep = factory(name57, dependencies58, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name57, {
...
var name58 = "factorial";
var dependencies59 = ["typed", "gamma"];
var createFactorial = factory(name58, dependencies59, (_ref) => {
  var {
    typed,
    gamma
  } = _ref;
...
var name59 = "multinomial";
var dependencies60 = ["typed", "add", "divide", "multiply", "factorial", "isInteger", "isPositive"];
var createMultinomial = factory(name59, dependencies60, (_ref) => {
  var {
    typed,
    add: add2,
    divide: divide2,
...
var name60 = "permutations";
var dependencies61 = ["typed", "factorial"];
var createPermutations = factory(name60, dependencies61, (_ref) => {
  var {
    typed,
    factorial
  } = _ref;
...
var name61 = "pickRandom";
var dependencies62 = ["typed", "config", "?on"];
var createPickRandom = factory(name61, dependencies62, (_ref) => {
  var {
    typed,
    config: config4,
    on
...
var createRandom = factory(name62, dependencies63, (_ref) => {
  var {
    typed,
    config: config4,
    on
...
var createRandomNumber = factory(name62, ["typed", "config", "?on"], (_ref2) => {
  var {
    typed,
    config: config4,
    on,
...
var name63 = "randomInt";
var dependencies64 = ["typed", "config", "?on"];
var createRandomInt = factory(name63, dependencies64, (_ref) => {
  var {
    typed,
    config: config4,
    on
...
var name64 = "equalScalar";
var dependencies65 = ["typed", "config"];
var createEqualScalar = factory(name64, dependencies65, (_ref) => {
  var {
    typed,
    config: config4
  } = _ref;
...
var createEqualScalarNumber = factory(name64, ["typed", "config"], (_ref2) => {
  var {
    typed,
    config: config4
  } = _ref2;
...
var name65 = "matAlgo03xDSf";
var dependencies66 = ["typed"];
var createMatAlgo03xDSf = factory(name65, dependencies66, (_ref) => {
  var {
    typed
  } = _ref;
...
var name66 = "matAlgo05xSfSf";
var dependencies67 = ["typed", "equalScalar"];
var createMatAlgo05xSfSf = factory(name66, dependencies67, (_ref) => {
  var {
    typed,
    equalScalar
  } = _ref;
...
var name67 = "matAlgo13xDD";
var dependencies68 = ["typed"];
var createMatAlgo13xDD = factory(name67, dependencies68, (_ref) => {
  var {
    typed
  } = _ref;
...
var name68 = "matrixAlgorithmSuite";
var dependencies69 = ["typed", "matrix"];
var createMatrixAlgorithmSuite = factory(name68, dependencies69, (_ref) => {
  var {
    typed,
    matrix
  } = _ref;
...
var name69 = "compare";
var dependencies70 = ["typed", "config", "matrix", "equalScalar", "BigNumber", "Fraction", "DenseMatrix", "concat"];
var createCompare = factory(name69, dependencies70, (_ref) => {
  var {
    typed,
    config: config4,
    equalScalar,
  ...
  return typed(name69, createCompareNumber({
    typed,
    config: config4
  }), {
    "boolean, boolean": function boolean_boolean(x, y) {
...
var createCompareNumber = factory(name69, ["typed", "config"], (_ref2) => {
  var {
    typed,
    config: config4
  } = _ref2;
...
var name70 = "compareNatural";
var dependencies71 = ["typed", "compare"];
var createCompareNatural = factory(name70, dependencies71, (_ref) => {
  var {
    typed,
    compare
  } = _ref;
...
var name71 = "compareText";
var dependencies72 = ["typed", "matrix", "concat"];
compareText.signature = "any, any";
var createCompareText = factory(name71, dependencies72, (_ref) => {
  var {
    typed,
    matrix,
    concat: concat2
  ...
  return typed(name71, compareText, matrixAlgorithmSuite({
    elop: compareText,
    Ds: true
  }));
});
var createCompareTextNumber = factory(name71, ["typed"], (_ref2) => {
  var {
    typed
  } = _ref2;
  return typed(name71, compareText);
});

// node_modules/mathjs/lib/esm/type/matrix/utils/matAlgo07xSSf.js
var name72 = "matAlgo07xSSf";
var dependencies73 = ["typed", "DenseMatrix"];
var createMatAlgo07xSSf = factory(name72, dependencies73, (_ref) => {
  var {
    typed,
    DenseMatrix
  } = _ref;
...
var createEqual = factory(name73, dependencies74, (_ref) => {
  var {
    typed,
    matrix,
    equalScalar,
  ...
  return typed(name73, createEqualNumber({
    typed,
    equalScalar
...
var createEqualNumber = factory(name73, ["typed", "equalScalar"], (_ref2) => {
  var {
    typed,
    equalScalar
  } = _ref2;
...
var name74 = "equalText";
var dependencies75 = ["typed", "compareText", "isZero"];
var createEqualText = factory(name74, dependencies75, (_ref) => {
  var {
    typed,
...
var name75 = "smaller";
var dependencies76 = ["typed", "config", "matrix", "DenseMatrix", "concat"];
var createSmaller = factory(name75, dependencies76, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
  ...
  return typed(name75, createSmallerNumber({
    typed,
    config: config4
  }), {
...
var createSmallerNumber = factory(name75, ["typed", "config"], (_ref2) => {
  var {
    typed,
    config: config4
  } = _ref2;
...
var name76 = "smallerEq";
var dependencies77 = ["typed", "config", "matrix", "DenseMatrix", "concat"];
var createSmallerEq = factory(name76, dependencies77, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
  ...
  return typed(name76, createSmallerEqNumber({
    typed,
    config: config4
  }), {
    "boolean, boolean": (x, y) => x <= y,
...
var createSmallerEqNumber = factory(name76, ["typed", "config"], (_ref2) => {
  var {
    typed,
    config: config4
  } = _ref2;
...
var name77 = "larger";
var dependencies78 = ["typed", "config", "matrix", "DenseMatrix", "concat"];
var createLarger = factory(name77, dependencies78, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
  ...
  return typed(name77, createLargerNumber({
    typed,
    config: config4
  }), {
    "boolean, boolean": (x, y) => x > y,
...
var createLargerNumber = factory(name77, ["typed", "config"], (_ref2) => {
  var {
    typed,
    config: config4
  } = _ref2;
...
var name78 = "largerEq";
var dependencies79 = ["typed", "config", "matrix", "DenseMatrix", "concat"];
var createLargerEq = factory(name78, dependencies79, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
  ...
  return typed(name78, createLargerEqNumber({
    typed,
    config: config4
  }), {
    "boolean, boolean": (x, y) => x >= y,
...
var createLargerEqNumber = factory(name78, ["typed", "config"], (_ref2) => {
  var {
    typed,
    config: config4
  } = _ref2;
...
var name79 = "deepEqual";
var dependencies80 = ["typed", "equal"];
var createDeepEqual = factory(name79, dependencies80, (_ref) => {
  var {
    typed,
    equal
  } = _ref;
...
var name80 = "unequal";
var dependencies81 = ["typed", "config", "equalScalar", "matrix", "DenseMatrix", "concat"];
var createUnequal = factory(name80, dependencies81, (_ref) => {
  var {
    typed,
    config: config4,
    equalScalar,
  ...
  return typed(name80, createUnequalNumber({
    typed,
    equalScalar
  }), matrixAlgorithmSuite({
    elop: _unequal,
...
var createUnequalNumber = factory(name80, ["typed", "equalScalar"], (_ref2) => {
  var {
    typed,
    equalScalar
  } = _ref2;
...
var createErf = factory(name81, dependencies82, (_ref) => {
  var {
    typed
  } = _ref;
  return typed("name", {
...
var name82 = "zeta";
var dependencies83 = ["typed", "config", "multiply", "pow", "divide", "factorial", "equal", "smallerEq", "isNegative", "gamma", "sin", "subtract", "add", "?Complex", "?BigNumber", "pi"];
var createZeta = factory(name82, dependencies83, (_ref) => {
  var {
    typed,
    config: config4,
    multiply,
...
var name83 = "mode";
var dependencies84 = ["typed", "isNaN", "isNumeric"];
var createMode = factory(name83, dependencies84, (_ref) => {
  var {
    typed,
    isNaN: isNaN2,
    isNumeric
...
var name84 = "prod";
var dependencies85 = ["typed", "config", "multiplyScalar", "numeric"];
var createProd = factory(name84, dependencies85, (_ref) => {
  var {
    typed,
    config: config4,
    multiplyScalar,
  ...
  function _prod(array) {
    ...
    if (typeof prod === "string") {
      prod = numeric2(prod, safeNumberType(prod, config4));
    }
    if (prod === void 0) {
...
var name85 = "max";
var dependencies86 = ["typed", "config", "numeric", "larger"];
var createMax = factory(name85, dependencies86, (_ref) => {
  var {
    typed,
    config: config4,
    numeric: numeric2,
  ...
  function _max(array) {
    ...
    if (typeof res === "string") {
      res = numeric2(res, safeNumberType(res, config4));
    }
    return res;
  }
});
...
var name86 = "min";
var dependencies87 = ["typed", "config", "numeric", "smaller"];
var createMin = factory(name86, dependencies87, (_ref) => {
  var {
    typed,
    config: config4,
    numeric: numeric2,
  ...
  function _min(array) {
    ...
    if (typeof min2 === "string") {
      min2 = numeric2(min2, safeNumberType(min2, config4));
    }
    return min2;
  }
});
...
var name87 = "sum";
var dependencies88 = ["typed", "config", "add", "numeric"];
var createSum = factory(name87, dependencies88, (_ref) => {
  var {
    typed,
    config: config4,
    add: add2,
...
var name88 = "cumsum";
var dependencies89 = ["typed", "add", "unaryPlus"];
var createCumSum = factory(name88, dependencies89, (_ref) => {
  var {
    typed,
    add: add2,
    unaryPlus
  ...
  function _cumsum(array) {
    try {
      ...
      throw improveErrorMessage(err, name88);
    }
  }
...
var name89 = "mean";
var dependencies90 = ["typed", "add", "divide"];
var createMean = factory(name89, dependencies90, (_ref) => {
  var {
    typed,
    add: add2,
    divide: divide2
...
var name90 = "median";
var dependencies91 = ["typed", "add", "divide", "compare", "partitionSelect"];
var createMedian = factory(name90, dependencies91, (_ref) => {
  var {
    typed,
    add: add2,
    divide: divide2,
...
var name91 = "mad";
var dependencies92 = ["typed", "abs", "map", "median", "subtract"];
var createMad = factory(name91, dependencies92, (_ref) => {
  var {
    typed,
    abs: abs2,
    map: map2,
...
var name92 = "variance";
var dependencies93 = ["typed", "add", "subtract", "multiply", "divide", "apply", "isNaN"];
var createVariance = factory(name92, dependencies93, (_ref) => {
  var {
    typed,
    add: add2,
    subtract,
...
var name93 = "quantileSeq";
var dependencies94 = ["typed", "?bignumber", "add", "subtract", "divide", "multiply", "partitionSelect", "compare", "isInteger", "smaller", "smallerEq", "larger"];
var createQuantileSeq = factory(name93, dependencies94, (_ref) => {
  var {
    typed,
    bignumber,
    add: add2,
...
var name94 = "std";
var dependencies95 = ["typed", "map", "sqrt", "variance"];
var createStd = factory(name94, dependencies95, (_ref) => {
  var {
    typed,
    map: map2,
    sqrt: sqrt2,
...
var name95 = "corr";
var dependencies96 = ["typed", "matrix", "mean", "sqrt", "sum", "add", "subtract", "multiply", "pow", "divide"];
var createCorr = factory(name95, dependencies96, (_ref) => {
  var {
    typed,
    matrix,
    sqrt: sqrt2,
...
var createFormat = factory(name96, dependencies97, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name96, {
...
var createPrint = factory(name97, dependencies98, (_ref) => {
  var {
    typed
  } = _ref;
  return typed(name97, {
...
var name98 = "apply";
var dependencies99 = ["typed", "isInteger"];
var createApplyTransform = factory(name98, dependencies99, (_ref) => {
  var {
    typed,
    isInteger: isInteger2
  } = _ref;
...
function compileInlineExpression(expression, math, scope) {
  ...
  var name118 = symbol.name;
  var argsScope = /* @__PURE__ */ new Map();
  var subScope = new PartitionedMap(scope, argsScope, /* @__PURE__ */ new Set([name118]));
  var eq = expression.compile();
  return function inlineExpression(x) {
    argsScope.set(name118, x);
...
var createFilterTransform = factory(name99, dependencies100, (_ref) => {
  var {
    typed
  } = _ref;
...
var name100 = "forEach";
var dependencies101 = ["typed"];
var createForEachTransform = factory(name100, dependencies101, (_ref) => {
  var {
    typed
  } = _ref;
  ...
  var _forEach2 = typed("forEach", {
    "Array | Matrix, function": function Array__Matrix_function(array, callback) {
      var _recurse2 = function recurse(value, index) {
        if (Array.isArray(value)) {
          forEach(value, function(child, i) {
            _recurse2(child, index.concat(i + 1));
          });
        } else {
          return applyCallback(callback, value, index, array, "forEach");
        }
      };
      _recurse2(array.valueOf(), []);
    }
  });
...
var name101 = "map";
var dependencies102 = ["typed"];
var createMapTransform = factory(name101, dependencies102, (_ref) => {
  var {
    typed
  } = _ref;
  var map2 = createMap2({
  ...
  function mapTransform(args, math, scope) {
    ...
    return map2(...X, _transformCallback(callback, N));
...
var name102 = "max";
var dependencies103 = ["typed", "config", "numeric", "larger"];
var createMaxTransform = factory(name102, dependencies103, (_ref) => {
  var {
    typed,
    config: config4,
    numeric: numeric2,
...
var name103 = "mean";
var dependencies104 = ["typed", "add", "divide"];
var createMeanTransform = factory(name103, dependencies104, (_ref) => {
  var {
    typed,
    add: add2,
    divide: divide2
...
var name104 = "min";
var dependencies105 = ["typed", "config", "numeric", "smaller"];
var createMinTransform = factory(name104, dependencies105, (_ref) => {
  var {
    typed,
    config: config4,
    numeric: numeric2,
...
var name105 = "range";
var dependencies106 = ["typed", "config", "?matrix", "?bignumber", "smaller", "smallerEq", "larger", "largerEq", "add", "isPositive"];
var createRangeTransform = factory(name105, dependencies106, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
...
var name106 = "std";
var dependencies107 = ["typed", "map", "sqrt", "variance"];
var createStdTransform = factory(name106, dependencies107, (_ref) => {
  var {
    typed,
    map: map2,
    sqrt: sqrt2,
...
var name107 = "sum";
var dependencies108 = ["typed", "config", "add", "numeric"];
var createSumTransform = factory(name107, dependencies108, (_ref) => {
  var {
    typed,
    config: config4,
    add: add2,
...
var name108 = "cumsum";
var dependencies109 = ["typed", "add", "unaryPlus"];
var createCumSumTransform = factory(name108, dependencies109, (_ref) => {
  var {
    typed,
    add: add2,
    unaryPlus
...
var name109 = "variance";
var dependencies110 = ["typed", "add", "subtract", "multiply", "divide", "apply", "isNaN"];
var createVarianceTransform = factory(name109, dependencies110, (_ref) => {
  var {
    typed,
    add: add2,
    subtract,
...
var createClone = factory(name110, dependencies111, (_ref) => {
  var {
    typed
  } = _ref;
...
var name111 = "isNumeric";
var dependencies112 = ["typed"];
var createIsNumeric = factory(name111, dependencies112, (_ref) => {
  var {
    typed
  } = _ref;
...
var name112 = "hasNumericValue";
var dependencies113 = ["typed", "isNumeric"];
var createHasNumericValue = factory(name112, dependencies113, (_ref) => {
  var {
    typed,
    isNumeric
  } = _ref;
...
var createTypeOf = factory(name113, dependencies114, (_ref) => {
  var {
    typed
  } = _ref;
...
var name114 = "isPrime";
var dependencies115 = ["typed"];
var createIsPrime = factory(name114, dependencies115, (_ref) => {
  var {
    typed
  } = _ref;
...
var name115 = "numeric";
var dependencies116 = ["number", "?bignumber", "?fraction"];
var createNumeric = factory(name115, dependencies116, (_ref) => {
  var {
    number: _number,
    bignumber,
    fraction
...
var name116 = "reviver";
var dependencies117 = ["classes"];
var createReviver = factory(name116, dependencies117, (_ref) => {
  var {
    classes
  } = _ref;
...
var name117 = "replacer";
var dependencies118 = [];
var createReplacer = factory(name117, dependencies118, () => {
...
function importFactory(typed, load, math, importedFactories) {
  function mathImport(functions, options) {
    ...
    function flattenImports(flatValues2, value2, name119) {
      ...
      } else if (isFactory(value2) || name119 !== void 0) {
        var flatName = isFactory(value2) ? isTransformFunctionFactory(value2) ? value2.fn + ".transform" : value2.fn : name119;
    ...
    for (var name118 in flatValues) {
      if (hasOwnProperty(flatValues, name118)) {
        var value = flatValues[name118];
        if (isFactory(value)) {
          _importFactory(value, options);
        } else if (isSupportedType(value)) {
          _import(name118, value, options);
        } else {
          if (!options.silent) {
            throw new TypeError("Factory, Object, or Array expected");
          }
  ...
  function _import(name118, value, options) {
    ...
    if (typed.isTypedFunction(math[name118]) && typed.isTypedFunction(value)) {
      if (options.override) {
        value = typed(name118, value.signatures);
      } else {
        value = typed(math[name118], value);
      }
      math[name118] = value;
      delete importedFactories[name118];
      _importTransform(name118, value);
      math.emit("import", name118, function resolver() {
        return value;
      });
      return;
    }
    if (math[name118] === void 0 || options.override) {
      math[name118] = value;
      delete importedFactories[name118];
      _importTransform(name118, value);
      math.emit("import", name118, function resolver() {
    ...
    if (!options.silent) {
      throw new Error('Cannot import "' + name118 + '": already exists');
    }
  }
  function _importTransform(name118, value) {
    if (value && typeof value.transform === "function") {
      math.expression.transform[name118] = value.transform;
      if (allowedInExpressions(name118)) {
        math.expression.mathWithTransform[name118] = value.transform;
      }
    } else {
      delete math.expression.transform[name118];
      if (allowedInExpressions(name118)) {
        math.expression.mathWithTransform[name118] = value;
      }
    }
  }
  function _deleteTransform(name118) {
    delete math.expression.transform[name118];
    if (allowedInExpressions(name118)) {
      math.expression.mathWithTransform[name118] = math[name118];
    } else {
      delete math.expression.mathWithTransform[name118];
    }
  }
  function _wrap(fn) {
  ...
  function _importFactory(factory2, options) {
    var name118 = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : factory2.fn;
    if (name118.includes(".")) {
      throw new Error("Factory name should not contain a nested path. Name: " + JSON.stringify(name118));
    }
    var namespace = isTransformFunctionFactory(factory2) ? math.expression.transform : math;
    var existingTransform = name118 in math.expression.transform;
    var existing = hasOwnProperty(namespace, name118) ? namespace[name118] : void 0;
    var resolver = function resolver2() {
      ...
      factory2.dependencies.map(stripOptionalNotation).forEach((dependency) => {
        ...
        if (dependency === "math") {
          dependencies119.math = math;
        } else if (dependency === "mathWithTransform") {
          dependencies119.mathWithTransform = math.expression.mathWithTransform;
        } else if (dependency === "classes") {
          dependencies119.classes = math;
        } else {
          dependencies119[dependency] = math[dependency];
        }
      });
      var instance = factory2(dependencies119);
    ...
    if (!factory2.meta || factory2.meta.lazy !== false) {
      lazy(namespace, name118, resolver);
      if (existing && existingTransform) {
        _deleteTransform(name118);
      } else {
        if (isTransformFunctionFactory(factory2) || factoryAllowedInExpressions(factory2)) {
          lazy(math.expression.mathWithTransform, name118, () => namespace[name118]);
        }
      }
    } else {
      namespace[name118] = resolver();
      if (existing && existingTransform) {
        _deleteTransform(name118);
      } else {
        if (isTransformFunctionFactory(factory2) || factoryAllowedInExpressions(factory2)) {
    ...
    importedFactories[name118] = factory2;
    math.emit("import", name118, resolver);
  }
  ...
  function allowedInExpressions(name118) {
    return !hasOwnProperty(unsafe, name118);
  }
...
decimal.js/decimal.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-XPGKCA64.js.map
